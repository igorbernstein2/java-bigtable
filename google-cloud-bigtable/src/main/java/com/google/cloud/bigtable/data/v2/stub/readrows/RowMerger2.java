 ...
 * ReadRowsResponse.CellChunk}s into this class and pops fully merged logical
 * rows. Example usage:
 *
 * <pre>{@code
 ...
 */
@InternalApi
public class RowMerger2<RowT> implements Reframer<RowT, ReadRowsResponse> {
  private final StateMachine<RowT> stateMachine;
  private Queue<RowT> mergedRows;

  public RowMerger2(RowBuilder<RowT> rowBuilder) {
  ...
  @Override
  public RowT pop() {
    return Preconditions.checkNotNull(mergedRows.poll(), "RowMerger.pop() called when there are no rows");
  }
}
